...
cd stelar_3dunet/
source spt19/bin/activate
python3 3D_unet_bias_mit_1_representation_class_weights_and_RGBLAI2_more_data.py --crop_1 1 --crop_2 2 --crop_3 3
...
for l in range(input_img.shape[0]):
    for i in range(64):
        for j in range(64):
                input_img[l, :, i, j]= replace_zeros_with_average(input_img[l, :, i, j])
...
# Define the VIBGYOR colors in RGB
vibgyor_colors = [
    [128, 0, 128],  # Violet
    [75, 0, 130],   # Indigo
    [0, 0, 255],    # Blue
...
def generate_vibgyor_color_mapping(num_colors, vibgyor_colors):
    # Convert VIBGYOR colors to a format suitable for interpolation
    vibgyor_colors = np.array(vibgyor_colors) / 255.0  # Normalize RGB values to [0, 1]
    
    # Create a colormap that transitions through VIBGYOR
    cmap = mcolors.LinearSegmentedColormap.from_list('vibgyor', vibgyor_colors, N=num_colors)
    
    # Generate colors
    color_mapping = {i: list((np.array(cmap(i)[:3]) * 255).astype(int)) for i in range(num_colors)}
    
    return color_mapping

color_mapping = generate_vibgyor_color_mapping(71, vibgyor_colors)
...
output_img = np.zeros((input_img.shape[0], input_img.shape[1], input_img.shape[2], input_img.shape[3], 3), dtype=np.uint8)


for unique_value, color in color_mapping.items():
    mask = (input_img == unique_value)
    for i in range(3):  # Iterate over the RGB channels
        output_img[..., i][mask] = color[i]
...
print("step here before : input_img.shape", input_img.shape)

#train_img = np.stack((input_img,)*3, axis=-1)
#train_img = np.stack((input_img,)*1, axis=-1)
#print("step here befafterore : train_img.shape", train_img.shape)


'''def apply_colormap_to_3d(img, colormap):
    # Initialize an empty array for the RGB image
    rgb_img = np.zeros((*img.shape, 3))
    # Flatten the 3D image to apply colormap
    flattened_img = img.flatten()
    # Apply colormap to the flattened image
    colored_flattened_img = colormap(flattened_img)[:, :3]  # Discard the alpha channel
    # Reshape back to the original 3D shape with 3 color channels
    rgb_img = colored_flattened_img.reshape((*img.shape, 3))
    return rgb_img
#colormap = cm.get_cmap('viridis')
colormap = plt.get_cmap('viridis')

train_img = np.array([(apply_colormap_to_3d(img, colormap)).astype(np.uint8) for img in input_img])
print("step here befafterore : train_img.shape", train_img.shape)
'''
train_img = input_img.astype(np.uint8)

train_lai_uniques = np.unique(train_img)
print("train_lai_uniques", train_lai_uniques)

del input_img
train_mask = np.expand_dims(input_mask, axis=4)
train_mask_cat = to_categorical(train_mask, num_classes=n_classes)

X_train, X_test, y_train, y_test = train_test_split(train_img, train_mask_cat, test_size = 0.05, random_state = 0)
...
model.save('/home/luser/stelar_3dunet/storage/saved_model_rgblai2/3D_unet_res_labels_'+vista_crop_dict[chosen_crop_types_list[0]]+'_'+vista_crop_dict[chosen_crop_types_list[1]]+'_'+vista_crop_dict[chosen_crop_types_list[2]]+'_num_epocs_'+str(num_epochs)+'.h5')
