...
if class_weights:
    ...
    #chosen_crop_types_list_list_models = [ [1, 11, 32], [4, 5, 7], [5, 28, 37], [8,  9, 10], [14, 15, 16], [18, 19, 20], [21, 23, 27], [28, 30, 32], [33, 34, 35], [33, 36, 37], [35, 37, 40], [11, 12, 13], [1, 2, 3], [8, 9, 41]]  # [37, 40, 41]
    #chosen_crop_types_list_list_models = [[8, 9, 41], [13, 19, 30], [1, 11, 32], [18, 33, 36]]  # [37, 40, 41]
    ...
    chosen_crop_types_list_list = [[34, 35, 37], [33, 36, 40], [33, 36, 41]]
...
for test_img_number in range(290):
    for chosen_subset_for_test_set in range(len(chosen_crop_types_list_list)):
        ...
        '''for k in crop_types_all_list:
            if (k in chosen_crop_types_list):
                y_test_1[y_test_1==k]=k
            else:
                y_test_1[y_test_1==k]=0'''

        #print("1: np.unique(ground_truth_1)", np.unique(ground_truth_1))

        for k in crop_types_all_list:
            if not(k in chosen_season_crops):
                ground_truth_1[ground_truth_1==k]=0

        #print("2: np.unique(ground_truth_1)", np.unique(ground_truth_1))
        ...
        tifffile.imsave('./ensamble_results/iou_f1_class_weights/inputs/input_subset_'+str(chosen_subset_for_test_set)+'_sample_'+str(test_img_number)+'_.tif', test_img)
        ...
        for chosen_crop_types_list in chosen_crop_types_list_list_models:
            ...
            gc.collect()
            if class_weights:
                #my_model_1 = load_model('./storage/data_gen_model/saved_models_max_LAI_corr/regular/3D_unet_res_labels_'+vista_crop_dict[chosen_crop_types_list[0]]+'_'+vista_crop_dict[chosen_crop_types_list[1]]+'_'+vista_crop_dict[chosen_crop_types_list[2]]+'_epoch_'+str(num_epochs)+'.h5', compile=False)
        ...
        if cloud_interpolation:
            tifffile.imsave('./ensamble_results/iou_f1_class_weights/cloud_interpol/ground_truth_subset_'+str(chosen_subset_for_test_set)+'_sample_'+str(test_img_number)+'_.tif', test_mode)
            tifffile.imsave('./ensamble_results/iou_f1_class_weights/cloud_interpol/prediction_subset_'+str(chosen_subset_for_test_set)+'_sample_'+str(test_img_number)+'_.tif', ground_truth_flattened)
        else:
            tifffile.imsave('./ensamble_results/iou_f1_class_weights/no_cloud_interpol/ground_truth_subset_'+str(chosen_subset_for_test_set)+'_sample_'+str(test_img_number)+'_.tif', test_mode)
            tifffile.imsave('./ensamble_results/iou_f1_class_weights/no_cloud_interpol/prediction_subset_'+str(chosen_subset_for_test_set)+'_sample_'+str(test_img_number)+'_.tif', ground_truth_flattened)
        ...
        for i in range(64):
            ...
            plt.axis('off')
        if cloud_interpolation:
            plt.savefig('./ensamble_results/after_class_weights_with_interpolation/LAI_subset_'+str(chosen_subset_for_test_set)+'_sample_'+str(test_img_number)+'_.png', bbox_inches='tight')
        else:
            plt.savefig('./ensamble_results/after_class_weights_without_interpolation/LAI_subset_'+str(chosen_subset_for_test_set)+'_sample_'+str(test_img_number)+'_.png', bbox_inches='tight')
        #plt.show()
        plt.close()
        ...
        if cloud_interpolation:
            plt.savefig('./ensamble_results/after_class_weights_with_interpolation/ground_truth_pred_subset_'+str(chosen_subset_for_test_set)+'_sample_'+str(test_img_number)+'_.png', bbox_inches='tight')
        else:
            plt.savefig('./ensamble_results/after_class_weights_without_interpolation/ground_truth_pred_subset_'+str(chosen_subset_for_test_set)+'_sample_'+str(test_img_number)+'_.png', bbox_inches='tight')
        #plt.show()
        plt.close()
