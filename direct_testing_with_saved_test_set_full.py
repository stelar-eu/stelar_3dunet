...
conda activate stcon4
cd stelar_3dunet/
python3 direct_testing_with_saved_test_set_full.py --season spring
...
conda activate stcon4
cd stelar_3dunet/
python3 direct_testing_with_saved_test_set_full.py --season summer_autumn
...
conda activate stcon4
cd stelar_3dunet/
python3 direct_testing_with_saved_test_set_full.py --season winter_spring_summer
...
for test_img_number in range(300):
    for chosen_subset_for_test_set in range(len(chosen_crop_types_list_list)):
        ...
        for chosen_crop_types_list in chosen_crop_types_list_list_models:
            ...
            print("what is the shape here ? ", test_img_input1.shape)
            test_pred1 = my_model_1.predict(test_img_input1)
            print("what isthe shape after test_pred1.shape", test_pred1.shape)
            del my_model_1
            gc.collect()
            ...
            plt.savefig("/data1/chethan/stelar_3dunet/general_tests/pre"+str(ll)+".png")
            ll+=1
            for i in range(3):
        ...
        for i in range(64):
            ...
            plt.axis('off')
        if cloud_interpolation:
            plt.savefig('./ensamble_results/after_class_weights_with_interpolation/'+season+'LAI_subset_'+str(chosen_subset_for_test_set)+'_sample_'+str(test_img_number)+'_.png', bbox_inches='tight')
        else:
            plt.savefig('./ensamble_results/after_class_weights_without_interpolation/'+season+'LAI_subset_'+str(chosen_subset_for_test_set)+'_sample_'+str(test_img_number)+'_.png', bbox_inches='tight')
        #plt.show()
        plt.close()
        ...
        if cloud_interpolation:
            plt.savefig('./ensamble_results/after_class_weights_with_interpolation/'+season+'ground_truth_pred_subset_'+str(chosen_subset_for_test_set)+'_sample_'+str(test_img_number)+'_.png', bbox_inches='tight')
        else:
            plt.savefig('./ensamble_results/after_class_weights_without_interpolation/'+season+'ground_truth_pred_subset_'+str(chosen_subset_for_test_set)+'_sample_'+str(test_img_number)+'_.png', bbox_inches='tight')
        #plt.show()
        plt.close()
